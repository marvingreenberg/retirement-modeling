...
function detectBroker(text: string, headers: string[]): BrokerFormat {
   ...
   if (lower.includes('fidelity') || headers.includes('last price change'))
      return 'fidelity';
   if (lower.includes('schwab') || headers.includes('day change %'))
      return 'schwab';
   if (lower.includes('vanguard') || headers.includes('investment name'))
      return 'vanguard';
   return 'generic';
}
...
function rowToHolding(row: CsvRow, broker: BrokerFormat): ParsedHolding | null {
   const symbol =
      getField(row, ['symbol', 'ticker']).replace(/[^A-Z0-9]/gi, '') || null;
   ...
   const marketValue = parseNumber(
      getField(row, ['current value', 'market value', 'total value', 'value']),
   );
   ...
   if (
      symbol === 'CASH' ||
      (lowerName.includes('pending') && lowerName.includes('activity'))
   )
      return null;
   ...
   for (const [accountId, accountRows] of groups) {
      ...
      for (const row of accountRows) {
         ...
         if (
            ...
            if (
               val > 0 &&
               !holdings.some((h) => h.name.toLowerCase() === name)
            ) {
               cash += val;
            }
         }
      ...
      accounts.push({
         account_id:
            accountId === 'default'
               ? '1'
               : accountId.replace(/[^A-Z0-9]/gi, '').slice(-6),
         broker: brokerName,
         account_type: null,
         holdings,
