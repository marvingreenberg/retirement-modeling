...
describe('fileIO', () => {
   describe('generateFilename', () => {
      it('uses both names when available', () => {
         expect(generateFilename('Marvin', 'Susie')).toBe(
            'Marvin-Susie-Retirement-Data.json',
         );
      });

      it('uses only primary name when no spouse', () => {
         expect(generateFilename('Marvin', '')).toBe(
            'Marvin-Retirement-Data.json',
         );
      });
      ...
      it('sanitizes special characters', () => {
         expect(generateFilename("O'Brien", 'Jane')).toBe(
            'OBrien-Jane-Retirement-Data.json',
         );
      });

      it('replaces spaces with dashes', () => {
         expect(generateFilename('Mary Jane', 'Bob')).toBe(
            'Mary-Jane-Bob-Retirement-Data.json',
         );
      });
   ...
   describe('saveJsonFile', () => {
      ...
      it('uses File System Access API when available', async () => {
         ...
         const mockCreateWritable = vi
            .fn()
            .mockResolvedValue({ write: mockWrite, close: mockClose });
