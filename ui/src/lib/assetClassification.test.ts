...
import { classifyHolding, summarizePortfolio, ASSET_CLASS_MAP } from './assetClassification';
...
describe('classifyHolding', () => {
	...
	it('classifies known intl equity ETF', () => {
		expect(classifyHolding(makeHolding({ symbol: 'VXUS' }))).toBe('intl_equity');
		expect(classifyHolding(makeHolding({ symbol: 'VEA' }))).toBe('intl_equity');
	});
	...
	it('classifies known intl bond ETF', () => {
		expect(classifyHolding(makeHolding({ symbol: 'BNDX' }))).toBe('intl_bond');
	});
	...
	it('defaults unknown stock to us_equity', () => {
		expect(classifyHolding(makeHolding({ symbol: 'ZZZZ', security_type: 'Stock' }))).toBe('us_equity');
	});

	it('defaults unknown MF to other', () => {
		expect(classifyHolding(makeHolding({ symbol: 'UNKN', security_type: 'MF' }))).toBe('other');
	});

	it('defaults null symbol stock to us_equity', () => {
		expect(classifyHolding(makeHolding({ symbol: null, security_type: 'Stock' }))).toBe('us_equity');
	});
...
describe('summarizePortfolio', () => {
	...
	it('computes correct allocation for simple case', () => {
		...
		const summary = summarizePortfolio(holdings, 0);
		const usEquity = summary.allocation.find((a) => a.assetClass === 'us_equity');
