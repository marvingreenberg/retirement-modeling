...
describe('WithdrawalPlan', () => {
	...
	it('shows year and age', () => {
		render(WithdrawalPlan, { props: { years: [makeYear({ year: 2027, age_primary: 66 })] } });
		expect(screen.getByText(/2027/)).toBeTruthy();
		expect(screen.getByText(/Age 66/)).toBeTruthy();
	});

	it('shows spending target', () => {
		render(WithdrawalPlan, { props: { years: [makeYear({ spending_target: 100000 })] } });
	...
	it('shows RMD section with per-account details', () => {
		const yr = makeYear({
			rmd: 20000,
			withdrawal_details: [
				{ account_id: 'ira1', account_name: 'Primary IRA', amount: 20000, purpose: 'rmd' },
			],
		});
	...
	it('shows spending withdrawals with per-account details', () => {
		const yr = makeYear({
			withdrawal_details: [
				{ account_id: 'brok', account_name: 'Brokerage', amount: 50000, purpose: 'spending' },
			],
		});
		render(WithdrawalPlan, { props: { years: [yr] } });
	...
	it('shows Roth conversion with per-account details', () => {
		const yr = makeYear({
			...
			withdrawal_details: [
				{ account_id: 'ira1', account_name: 'Main IRA', amount: 80000, purpose: 'conversion' },
			],
		});
		render(WithdrawalPlan, { props: { years: [yr] } });
