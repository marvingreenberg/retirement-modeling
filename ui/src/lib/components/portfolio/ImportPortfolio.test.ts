...
vi.mock('$lib/ofxParser', () => ({
	parseOFX: vi.fn(() => [{
		account_id: 'OFX123', broker: 'Vanguard', account_type: null,
		holdings: [], total_value: 50000, cash_balance: 50000, as_of_date: null,
	}]),
}));
vi.mock('$lib/csvParser', () => ({
	parseCSV: vi.fn(() => [{
		account_id: 'CSV456', broker: 'Fidelity', account_type: null,
		holdings: [], total_value: 30000, cash_balance: 30000, as_of_date: null,
	}]),
}));
...
describe('ImportPortfolio', () => {
	...
	it('shows modal with combined accounts after multi-file upload', async () => {
		...
		const ofxFile = new File(['<OFX>data</OFX>'], 'portfolio.ofx', { type: 'text/plain' });
		const csvFile = new File(['Symbol,Shares\nVTI,100'], 'holdings.csv', { type: 'text/csv' });
