...
<div class="flex flex-col gap-3">
   <div class="flex gap-3 items-end">
      ...
      <span class="text-xs text-surface-500 self-center"
         >{currency(Math.round(config.annual_spend_net / 12))}/mo</span
      >
   </div>

   {#if plannedExpenses.length > 0}
      <div
         class="flex gap-3 items-end px-3 text-xs font-medium text-surface-500 dark:text-surface-400"
      >
   ...
   {#each plannedExpenses as expense, i (i)}
      ...
      <div
         class="flex gap-3 items-center p-3 bg-surface-100 dark:bg-surface-800 rounded flex-wrap"
      >
         <input
            type="text"
            class="input w-28 no-spinner"
            bind:value={plannedExpenses[i].name}
            onfocus={(e) => e.currentTarget.select()}
         ...
         <input
            type="number"
            class="input w-24 no-spinner {amountError
               ? 'ring-2 ring-error-500 border-error-500'
               : ''}"
            bind:value={plannedExpenses[i].amount}
            onfocus={(e) => e.currentTarget.select()}
            min="1"
            step="100"
            aria-label="Amount"
         ...
         <div class="w-44">
            {#if expense.expense_type === 'one_time'}
               <input
                  type="number"
                  class="input w-20 no-spinner {yearError
                     ? 'ring-2 ring-error-500 border-error-500'
                     : ''}"
                  bind:value={plannedExpenses[i].year}
                  onfocus={(e) => e.currentTarget.select()}
                  min="2000"
                  aria-label="Year"
               />
            {:else}
               <span class="flex items-center gap-1">
                  <input
                     ...
                     bind:value={plannedExpenses[i].start_year}
                     onfocus={(e) => e.currentTarget.select()}
                     min="2000"
                  ...
                  <input
                     type="number"
                     class="input w-20 no-spinner"
                     bind:value={plannedExpenses[i].end_year}
                     onfocus={(e) => e.currentTarget.select()}
                     min="2000"
                     aria-label="End Year"
                  />
         ...
         <div class="w-10 flex justify-center">
            <input
               type="checkbox"
               class="checkbox"
               bind:checked={plannedExpenses[i].inflation_adjusted}
   ...
   {/each}
   <button class="btn preset-tonal self-start" onclick={addExpense}
      >+ Add Expense</button
   >
</div>
