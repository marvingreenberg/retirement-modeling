...
describe('IncomeEditor', () => {
   ...
   it('renders Social Security section', () => {
      ...
      expect(
         screen.getByText('Primary FRA Benefit ($/yr)'),
      ).toBeInTheDocument();
   ...
   it('hides spouse SS fields when no spouse', () => {
      ...
      expect(
         screen.queryByText('Spouse FRA Benefit ($/yr)'),
   ...
   it('shows warning for start age past simulation end', () => {
      renderEditor(
         { current_age_primary: 65, simulation_years: 30, start_year: 2026 },
         [
            {
               name: 'Late',
               amount: 1000,
   ...
   it('displays COLA rate and taxable_pct as percentages', () => {
      ...
      const taxableInput = screen.getByLabelText(
         'Taxable %',
      ) as HTMLInputElement;
