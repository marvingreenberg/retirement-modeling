<script lang="ts">
	...
	import { helpTopics, getTopicById, getDefaultTopicId } from '$lib/helpContent';
		...
		>
			...
			<div class="flex items-center justify-between p-4 border-b border-surface-200 dark:border-surface-700 shrink-0">
				...
				<div class="flex items-center gap-1">
					...
					<button class="btn btn-sm preset-ghost" onclick={() => open = false} aria-label="Close help">
						<X size={18} />
					</button>
				</div>
			</div>
			...
			<div class="flex flex-1 overflow-hidden" class:flex-col={!maximized}>
					...
					{#if !maximized}
						<div class="flex gap-1 flex-wrap">
							{#each helpTopics as topic}
								<button
									class="btn btn-sm {currentTopicId === topic.id ? 'preset-filled' : 'preset-ghost'}"
					...
					{:else}
						<ul class="space-y-1">
							{#each helpTopics as topic}
								<li>
									<button
										class="w-full text-left text-sm px-3 py-2 rounded {currentTopicId === topic.id
				...
				<div class="flex-1 overflow-y-auto p-5">
					{#if currentTopic}
						<h3 class="text-base font-semibold text-surface-900 dark:text-surface-50 mb-4">{currentTopic.title}</h3>
