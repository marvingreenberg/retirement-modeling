...
describe('HelpDrawer', () => {
	...
	it('shows default topic for root route (Spending Strategies)', () => {
		render(HelpDrawer, { props: { open: true } });
		expect(screen.getByRole('heading', { name: 'Spending Strategies', level: 3 })).toBeInTheDocument();
	...
	it('navigates to a different topic on click', async () => {
		render(HelpDrawer, { props: { open: true } });
		const nav = screen.getByRole('navigation', { name: 'Help topics' });
		await fireEvent.click(within(nav).getByText('Tax Bracket Inflation Indexing'));
		expect(screen.getByRole('heading', { name: 'Tax Bracket Inflation Indexing', level: 3 })).toBeInTheDocument();
	});
