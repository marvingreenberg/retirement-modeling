<script lang="ts">
	...
	let coverageYears = $derived(spending > 0 ? Math.round(totalBalance / spending) : null);
	let hasAccounts = $derived($portfolio.accounts.length > 0);
</script>

<div class="flex flex-col items-center justify-center text-center py-16 px-8 space-y-4">
	...
	{#if hasAccounts}
		<div class="flex gap-6 text-sm text-surface-600 dark:text-surface-400 mt-2">
