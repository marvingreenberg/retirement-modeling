...
{#if collapsed}
	<div class="flex items-center gap-3 bg-surface-100 dark:bg-surface-800 rounded cursor-pointer" role="button" tabindex="0" onclick={toggleCollapsed} onkeydown={(e) => e.key === 'Enter' && toggleCollapsed()}>
		<button class="btn btn-sm preset-tonal flex-shrink-0" onclick={(e) => { e.stopPropagation(); onrun(); }} disabled={loading} aria-label="Run simulation">
			{#if loading}<span class="inline-block animate-spin">⏳</span>{:else}Simulate{/if}
		</button>
...
{:else}
	<div class="bg-surface-100 dark:bg-surface-800 rounded p-2 space-y-1.5" onkeydown={handleKeydown} role="group">
		<div class="flex gap-4 flex-wrap items-end">
			<button class="btn btn-sm preset-tonal self-end" onclick={onrun} disabled={loading} aria-label="Run simulation">
				{#if loading}<span class="inline-block animate-spin">⏳</span>{:else}Simulate{/if}
			</button>
