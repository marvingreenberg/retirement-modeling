...
{#if collapsed}
   <div
      class="flex items-center gap-3 bg-surface-100 dark:bg-surface-800 rounded cursor-pointer"
      role="button"
      tabindex="0"
   ...
   >
      <button
         class="btn btn-lg preset-tonal flex-shrink-0"
         onclick={(e) => {
            e.stopPropagation();
            onrun();
         }}
      ...
      <span class="text-sm text-surface-600 dark:text-surface-400 flex-1 py-2"
         >{summaryText}</span
      >
         ...
         >
            ...
            <input
               ...
               oninput={(e) =>
                  setPct(
                     e,
                     (v) => ($portfolio.config.investment_growth_rate = v),
                  )}
               min="-50"
               max="50"
               step="0.5"
