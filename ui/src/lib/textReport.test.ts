...
function makeYear(overrides: Partial<YearResult> = {}): YearResult {
	return {
		...
		conversion_tax_from_brokerage: 0, pretax_withdrawal: 50000,
		roth_withdrawal: 20000, brokerage_withdrawal: 30000, total_tax: 15000,
		irmaa_cost: 0, total_balance: 485000, spending_target: 100000,
		planned_expense: 0, total_income: 36000, income_tax: 12800,
		pretax_balance: 350000, roth_balance: 80000, roth_conversion_balance: 0,
		brokerage_balance: 55000, withdrawal_details: [], income_details: [],
		...overrides,
	};
}
...
function makeSim(years: YearResult[]): SimulationResponse {
	return {
		result: { strategy: 'standard', spending_strategy: 'fixed_dollar', years },
		summary: {
			final_balance: years[years.length - 1]?.total_balance ?? 0,
			total_taxes_paid: 15000, total_irmaa_paid: 0,
			total_roth_conversions: 10000, simulation_years: years.length,
...
describe('generateTextReport', () => {
	...
	it('shows withdrawal details in plan section', () => {
		const yr = makeYear({
			rmd: 25000,
			withdrawal_details: [
				{ account_id: 'a1', account_name: '401(k)', amount: 25000, purpose: 'rmd' },
				{ account_id: 'a2', account_name: 'Brokerage', amount: 30000, purpose: 'spending' },
			],
		});
	...
	it('shows IRMAA and conversion tax in plan section', () => {
		const report = generateTextReport(makeSim([makeYear({ irmaa_cost: 1200, conversion_tax: 2200 })]));
	...
	it('includes all year-by-year columns', () => {
		...
		const headerLine = lines.find((l) => l.includes('Year') && l.includes('AGI'));
	...
	it('summary shows strategy and final balance', () => {
		const report = generateTextReport(makeSim([makeYear({ total_balance: 1234567 })]));
	...
	it('omits income section when no income details', () => {
		const report = generateTextReport(makeSim([makeYear({ total_income: 0, income_details: [] })]));
