...
export function preprocessOFX(raw: string): string {
   ...
   if (ofxStart === -1)
      throw new Error('Not a valid OFX file: no <OFX> tag found');
...
export function parseOFX(content: string): ParsedAccount[] {
   ...
   if (parseError) {
      throw new Error(
         'File does not appear to be a valid OFX/QFX investment account export.',
      );
   }
...
function buildSecurityMap(
   doc: Document,
): Map<string, { ticker: string | null; name: string }> {
