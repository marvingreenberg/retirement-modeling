...
import { portfolioSchema, accountSchema, simulationConfigSchema, socialSecuritySchema, guardrailsConfigSchema, plannedExpenseSchema } from './schema';
...
describe('accountSchema', () => {
	...
	it('rejects invalid account type', () => {
		const result = accountSchema.safeParse({ ...validAccount, type: 'savings' });
		expect(result.success).toBe(false);
	});

	it('rejects cost basis ratio > 1', () => {
		const result = accountSchema.safeParse({ ...validAccount, cost_basis_ratio: 1.5 });
		expect(result.success).toBe(false);
	});
...
describe('socialSecuritySchema', () => {
	it('accepts valid config', () => {
		expect(socialSecuritySchema.safeParse(validSocialSecurity).success).toBe(true);
	});

	it('rejects start age below 62', () => {
		const result = socialSecuritySchema.safeParse({ ...validSocialSecurity, primary_start_age: 60 });
		expect(result.success).toBe(false);
	});

	it('rejects start age above 70', () => {
		const result = socialSecuritySchema.safeParse({ ...validSocialSecurity, spouse_start_age: 72 });
		expect(result.success).toBe(false);
	});
});
...
describe('simulationConfigSchema', () => {
	...
	it('rejects age above 120', () => {
		const result = simulationConfigSchema.safeParse({ ...validConfig, current_age_primary: 150 });
		expect(result.success).toBe(false);
	});

	it('rejects simulation years above 100', () => {
		const result = simulationConfigSchema.safeParse({ ...validConfig, simulation_years: 200 });
		expect(result.success).toBe(false);
	});

	it('rejects negative annual spend', () => {
		const result = simulationConfigSchema.safeParse({ ...validConfig, annual_spend_net: -5000 });
		expect(result.success).toBe(false);
	});

	it('rejects invalid spending strategy', () => {
		const result = simulationConfigSchema.safeParse({ ...validConfig, spending_strategy: 'unknown' });
		expect(result.success).toBe(false);
	});

	it('rejects state tax rate above 0.2', () => {
		const result = simulationConfigSchema.safeParse({ ...validConfig, tax_rate_state: 0.5 });
		expect(result.success).toBe(false);
	});
});
...
describe('portfolioSchema', () => {
	it('accepts valid portfolio', () => {
		const result = portfolioSchema.safeParse({ config: validConfig, accounts: [validAccount] });
		expect(result.success).toBe(true);
	});
