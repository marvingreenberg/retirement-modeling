...
test.describe('Details Page', () => {
	...
	test('shows year-by-year table after simulation', async ({ page }) => {
		...
		await expect(page.getByRole('columnheader', { name: 'AGI', exact: true })).toBeVisible();
		await expect(page.getByRole('columnheader', { name: 'Total Balance', exact: true })).toBeVisible();
	});
});
