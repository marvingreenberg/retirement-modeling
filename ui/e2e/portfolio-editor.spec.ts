...
test.describe('Portfolio Editor UX', () => {
   ...
   test('expense type change immediately switches When field', async ({
      page,
   }) => {
      ...
      const yearCountBefore = await page
      ...
      await expect(page.getByLabel('Year', { exact: true })).toHaveCount(
         yearCountBefore + 1,
      );
      ...
      await expect(page.getByLabel('Start Year')).toHaveCount(
         startYearCountBefore + 1,
      );
      await expect(page.getByLabel('Year', { exact: true })).toHaveCount(
         yearCountBefore,
      );
   ...
   test('select-on-focus replaces value when typing', async ({ page }) => {
      ...
      const balanceInput = page
         .getByRole('spinbutton', { name: 'Balance' })
         .first();
