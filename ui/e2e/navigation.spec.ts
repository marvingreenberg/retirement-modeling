...
test.describe('Navigation', () => {
   ...
   test('AppBar shows all nav links', async ({ page }) => {
      ...
      await expect(
         banner.getByRole('link', { name: /overview/i }),
      ).toBeVisible();
      await expect(
         banner.getByRole('link', { name: /compare/i }),
      ).toBeVisible();
      await expect(
         banner.getByRole('link', { name: /details/i }),
      ).toBeVisible();
   });
   ...
   test('avatar opens dropdown with section links and toggles', async ({
      page,
   }) => {
      await page.getByLabel('Open profile').click();
      await expect(
         page.getByRole('link', { name: /basic info/i }),
      ).toBeVisible();
      await expect(
         page.getByRole('link', { name: /load \/ save/i }),
      ).toBeVisible();
      await expect(
         page.getByRole('link', { name: /advanced settings/i }),
      ).toBeVisible();
   ...
   test('dropdown Advanced Settings link navigates to settings', async ({
      page,
   }) => {
      await page.getByLabel('Open profile').click();
