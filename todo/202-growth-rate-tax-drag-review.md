...
3. **Import-derived growth rate**: The import sets the global growth rate from a weighted average of all imported accounts. If accounts have very different allocations (e.g., aggressive brokerage + conservative bond IRA), a single blended rate may not represent either well.

4. **stock_pct scope**: Currently `stock_pct` only affects tax drag. Should it also influence the account's growth rate? e.g., `rate = stock_pct * equity_return + (1 - stock_pct) * bond_return`. This would make single-run mode more realistic but adds complexity.
