...
A Dockerfile SHALL build and run the FastAPI backend service.
...
A Dockerfile SHALL run the SvelteKit dev server with API proxy configured to reach the API container.
...
A `compose.yaml` file SHALL define both services and a shared network, compatible with Docker Compose and Podman Compose.
...
The Makefile SHALL include targets for common container operations.
